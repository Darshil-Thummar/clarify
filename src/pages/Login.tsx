...
const Login = () => {
    ...
    onSuccess: (resp) => {
        ...
        if (typeof token !== 'string' || token.trim().length === 0) {
            toast({ title: "Login failed", description: "Missing token from server." });
            return;
        }
        setAuthToken(token);
        ...
        <Card className="w-full max-w-md border shadow-soft">
            ...
            <CardContent className="space-y-4">
                <div className="space-y-2">
                    ...
                    <Input id="ue" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="sujal_dev or sujal@example.com" autoComplete="username" />
                </div>
                <div className="space-y-2">
